<?php
   class TopicController extends CI_Controller
   {
       ...
       public function topicupdate()
       {
		    ...
		    else
		    {
		         if ($this->upload->do_upload('image'))
		         {
		        	...
		        	$data = array('name' => $this->input->post('name'),'title' => $this->input->post('title'),'email' => $this->input->post('email'),'description' =>$this->input->post('description'),'phoneno' => $this->input->post('phno'),'linkedin' => $this->input->post('linkedin'),'facebook' => $this->input->post('facebook'),'twitter' => $this->input->post('twitter'),'userid' => $userid,'image' => $image_data['file_name']);
		   		    $data = $this->topicsmodel->updatetopic($data,$topicid);
		         ...
		         else
		         {
		         	$userid = $this->session->userdata('userid');
		         	$topicid = $this->input->post('topicid');
		    		$data = array('name' => $this->input->post('name'),'title' => $this->input->post('title'),'email' => $this->input->post('email'),'description' =>$this->input->post('description'),'phoneno' => $this->input->post('phno'),'linkedin' => $this->input->post('linkedin'),'facebook' => $this->input->post('facebook'),'twitter' => $this->input->post('twitter'),'userid' => $userid,'image' => $this->input->post('photo'));
		   		    $data = $this->topicsmodel->updatetopic($data,$topicid);
       ...
       {
       	 $id = $this->uri->segment(2);
       	 $deldata = $this->topicsmodel->topicdel($id);
       	 if($deldata)
       	 {
            $this->session->set_flashdata('sucess', 'Your knowledge Deleted Sucessfully');
			redirect('home');
       	 }
       }
